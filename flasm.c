...
static void finalizeTemporaryFile(char *targetName)
{
	FILE* targetFile;

	if (!backupCreated) {
		backupName = mstrdup(targetName);
		backupCreated = 1;
		strcpy(backupName + strlen(backupName) - 4, ".$wf");
		/* remove evtl. existing previous backup foo.$wf */
		remove(backupName);
		/* foo.swf -> foo.$wf */
		if (rename(targetName, backupName) != 0)
			tellUser(1, "couldn't update: file %s is in use", targetName);
	}
	else {
...
static void usage(void)
{
	...
	puts("");
	puts("");
	puts("(c) 2001 Opaque Industries, (c) 2002-2009 Igor Kogan, (c) 2005 Wang Zhen");
